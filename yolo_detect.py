...
parser.add_argument('--record', help='Record results from video or webcam and save it as "demo1.avi". Must specify --resolution argument to record.',
                    action='store_true')
parser.add_argument('--save_image', help='Save the processed image with bounding boxes to a file.',
                    action='store_true')  # Add this argument
...
record = args.record
save_image = args.save_image  # Get the value of the --save_image argument
...
while True:
    ...
    for i in range(len(detections)):
        ...
        xmin, ymin, xmax, ymax = xyxy.astype(
            int)  # Extract individual coordinates and convert to int
    ...
    if source_type == 'video' or source_type == 'usb' or source_type == 'picamera':
        cv2.putText(frame, f'FPS: {avg_frame_rate:0.2f}', (10, 20),
                    cv2.FONT_HERSHEY_SIMPLEX, .7, (0, 255, 255),
                    2)  # Draw framerate
    ...
    # Save the image if the --save_image flag is set
    if save_image:
        # Generate a unique filename using a timestamp
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        output_filename = f"detected_image_{timestamp}.jpg" if source_type in (
            'image', 'folder') else f"detected_video_frame_{timestamp}.jpg"
    ...
    if key == ord('q') or key == ord('Q'):  # Press 'q' to quit
        break
            ...
            'S'):  # Press 's' to pause inference
        cv2.waitKey()
    elif key == ord('p') or key == ord(
            'P'):  # Press 'p' to save a picture of results on this frame
